...
// 반복문을 사용해 배열에서 가장 가격이 높은 상품 객체를 찾아보고,
// 마지막에 가장 비싼 상품의 이름을 출력하세요.
...
for (let i = 1; i < products.length; i++) {
  ...
  // 최고 기록 유지 및 업데이트
  if (currentProduct.price > expensiveProduct.price) {
    expensiveProduct = currentProduct;
  }
}
console.log("⭐미션1⭐");
console.log(`가장 비싼 상품은 ${expensiveProduct.name} 입니다.`);
...
console.log(`망고가 배열 안에 있나요? ${fruits.includes("망고")}`);
console.log(
  `포도가 처음 등장하는 index는 ${fruits.findIndex(
    (el) => el === "포도"
  )} 입니다.`
);
const filterApple = fruits.filter((fruit) => {
  return fruit === "사과";
});
console.log(filterApple);

const firstThreeItems = fruits.slice(0, 3);
