// 미션1. API 호출1

// 아래와 같은 fakeApiCall 함수가 있습니다. 이 함수는 2초 후에 사용자 정보를 반환합니다.
// async와 await를 사용하여 이 API 호출을 기다리고, 사용자 이름을 콘솔에 출력하는 fetchUserData 함수를 작성하세요.

function fakeApiCall() {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({ name: "John Doe", age: 30 });
    }, 2000);
  });
}

// fetchUserData 함수 완성
async function fetchUserData() {
  // 1. fakeApiCall()이 Promise를 반환하므로 await를 사용하여
  // 결과가 올 때까지 기다립니다.
  try {
    const user = await fakeApiCall();
    // 2. 반환된 객체에서 name 속성을 콘솔에 출력
    console.log(user.name);
  } catch (error) {
    // 실무 습관: 에러 발생 시 처리 로직도 함께 넣어주는 것이 좋습니다.
    console.error("데이터 로딩 중 에러 발생:", error);
  }
}

fetchUserData();

// 미션2. API 호출2
// 아래의 API 주소는 포멧몬들의 정보가 담겨있는 data를 반환합니다.
// 주어진 API 주소를 사용해, 아래의 조건을 모두 만족하는 코드를 작성하세요.
// try/catch를 사용한 에러 핸들링
// async/await를 사용한 비동기 처리
// 받아온 데이터들 중 "color 값이 green"인 포켓몬의 정보들이 담긴 배열 출력
const API_URL = "https://pokemon-api-ecru-eta.vercel.app";

async function getPocketmonInfo() {
  try {
    const response = await fetch(API_URL);
    const { data } = await response.json();
    console.log(data);
    const greenPokemon = data.filter((pokemon) => pokemon.color === "green");
    console.log(greenPokemon);
  } catch (error) {
    console.error("에러 발생", error);
  }
}

getPocketmonInfo();
